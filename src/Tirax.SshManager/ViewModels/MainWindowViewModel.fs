...
module private Helper =
    let validate name server_port local_port destination :Result<TunnelConfigViewModel,string> =
        ...
        if ssh_server.IsNone then
            ...
            if remote_server.IsNone then
                ...
                Ok <| TunnelConfigViewModel( Name = name,
                                    SshHost = ssh_host,
                                    SshPort = ssh_port,
...
type MainWindowViewModel private () as model =
    ...
    let mutable new_tunnel = Unchecked.defaultof<ObservableAsPropertyHelper<TunnelConfigViewModel>>
    ...
    let tunnels = ObservableCollection<TunnelConfigViewModel>()
